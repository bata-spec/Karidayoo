...
既にこの形式で資料を作っている場合、以下2点を踏まえて内容を見直してください。

1. **カテゴリが「/」区切りで階層ブラウジングできるようになった**
   `category` に `"用語/概念/クリフォト/ガミコト"` のように「/」を使ってパスを書くと、
   アプリの「カテゴリで見る」がフォルダのようにドリルダウンできるツリー表示になる
   (詳細は下の `category` フィールドの説明を参照)。
   **注意:** 既に `category` に「/」を含む名前を使っている場合(例:単なる名前として
   `"用語/概念"` と書いていた場合)、今後はそれが2階層(`用語` → `概念`)として解釈される。
   1つのカテゴリ名として扱いたいなら「/」を使わない名前に変更するか、
   逆に階層化したいなら意図的に「/」区切りのパスへ書き換えること。
2. **既存作品をまるごと更新インポートできるようになった**
   アプリ側の作品ダッシュボードに「ファイルで更新」ボタンが追加され、同じ作品に対して
   新しいZIP/JSONを読み込ませると、その作品の全エントリ・テンプレートが新しい内容で
...
- `category`: 自由文字列(「キャラクター」「国」「時代」など)。
  「/」区切りで書くと階層として扱われる(例:`"用語/概念/クリフォト"` は
  用語 → 概念 → クリフォト の3階層)。階層化する必要がなければ「/」を含めなければ
  従来通り1つのフラットなカテゴリ名として扱われる。
